...
namespace tell::argct
{
  ...
  template<typename Args>
    ...
    {  
      ...
      // print usage info
      void operator()(std::ostream&, const char* prognm, int indent = 8) const;
    private:
...
template<typename Args>
void tell::argct::Getopt<Args>::operator()
(std::ostream& ost, const char* progname, int indent) const
{
  Ios_saver guard(std::cerr);
  ost << "usage: " << progname << '\n';
  std::apply([&ost,indent](const auto&... a) {
      ((ost << std::setw(indent) << std::right << std::get<0>(a)
	<< " <" << std::get<1>(a)
