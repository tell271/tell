...
namespace tell
{
  ...
  template<typename T, std::size_t N, typename X>
    std::array<T,N> array_cast(const X&);
  
  template<typename T, std::size_t N, typename U>
    std::array<T,N> array_cast(const std::array<U,N>& v)
    {
      std::array<T, N> r;
  ...
  template<typename T, std::size_t N, typename U>
    std::array<T,N> array_cast(const std::vector<U>& v)
    {
      assert(v.size() == N);
      std::array<T, N> r;
      std::copy(v.begin(), v.end(), r.begin());
  ...
  template<typename C, typename... Cs>
    auto make_array(C c, Cs... cs)
  {
    C a[] = {c, cs...};
    return std::array<C, sizeof(a)/sizeof(C)>{c, cs...};
  }
  ...
  //
  // stream input output for std::arrays
  //
  template<typename C, std::size_t N>
    std::ostream& operator<<(std::ostream&, const std::array<C,N>&);
  template<typename C, std::size_t N>
    std::istream& operator>>(std::istream&, std::array<C,N>&);  
}
...
template<typename C, std::size_t N>
  std::ostream& tell::operator<<(std::ostream& ost, const std::array<C,N>& u)
{
  ost << "[";
  bool first_time{true};
  for (auto x : u) {
  ...
  // the code below fails for arrays of array, cf.
  // https://stackoverflow.com/questions/5355081/
  //    why-cant-i-instantiate-operatorostream-vectort-with-t-vectorint
  // std::copy(u.begin(), u.end(), std::ostream_iterator<C>(ost, ", "));
  return ost << "]";
}

template<typename C, std::size_t N>
  std::istream& tell::operator>>(std::istream& ist, std::array<C,N>& u)
{
  char c;
  if (!(ist >> c)) {
    return ist;
